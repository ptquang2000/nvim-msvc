...
Remove all implicit solution scanning. `solution_candidates` is now populated in exactly two ways:
...
2. **`BufEnter *.sln`** — opening a `.sln` buffer (directly or via netrw) adds it to `solution_candidates` and unconditionally calls `set_solution()`.

The `Msvc:discover_solution()` method, the netrw directory `BufEnter` autocmd, and the `:Msvc discover` subcommand are all removed.

## Consequences

- No startup scanning cost. The plugin is instant in large monorepos.
- `solution_candidates` only contains solutions the user has explicitly visited. `:Msvc solution <tab>` is short and intentional.
- A user opening nvim in a directory with multiple `.sln` files sees no auto-selection; they open one directly or use `:Msvc solution <path>`.
